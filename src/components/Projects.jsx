        ...
        <div className="grid sm:grid-cols-2 gap-5 items-stretch">
          {projects.map((p, i) => (
            <Reveal key={p.name} delay={i * 0.08} className="h-full">
              ...
              >
                <div className="relative aspect-[16/9] overflow-hidden shrink-0 bg-ink-900 p-4">
                  <img
                    src={p.image}
                    alt={p.name}
                    className="w-full h-full object-contain rounded-lg border border-white/10 shadow-lg transition-transform duration-500 group-hover:scale-105"
                  />
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-ink-950/50 backdrop-blur-[2px]">
